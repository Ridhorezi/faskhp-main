...
<div>
    <div class="card shadow">
        <div class="card-header">
            Table Kerja Kuliah
            <div>
                <button class="btn btn-primary" onclick="location.href='{{ route('table.kerja-kuliah.add') }}'"><i
                        class="fa fa-plus p-1"></i> Create</button>
                @include('livewire.tables.exportKerjaKuliah')
                        ...
                        @foreach ($dataKerjaKuliah as $kerjaKuliah)
                            <tr>
                                ...
                                <td class="text-center">
                                    {{ ($dataKerjaKuliah->currentpage() - 1) * $dataKerjaKuliah->perpage() + $loop->index + 1 }}
                                </td>
                                <td class="text-center">{{ $kerjaKuliah->name }}</td>
                                <td class="text-center">{{ $kerjaKuliah->jenis_kelamin == 'l' ? 'Pria' : 'Wanita' }}
                                </td>
                                <td class="text-center">{{ $kerjaKuliah->nama_perusahaan }}</td>
                                <td class="text-center">{{ $kerjaKuliah->jabatan }}</td>
                                <td class="text-center">{{ $kerjaKuliah->tahun_kerja }}</td>
                                <td class="text-center">{{ $kerjaKuliah->nama_universitas }}</td>
                                <td class="text-center">{{ $kerjaKuliah->jurusan }}</td>
                                <td class="text-center">
                                    @if ($kerjaKuliah->dibuat == 1)
                                        {{ 'Administrator' }}
                                    @else
                                        {{ $kerjaKuliah->dibuat }}
                                    @endif
                                </td>
                                <td class="text-center">{{ $kerjaKuliah->created_at->diffForHumans() }}</td>
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <img src="{{ asset('storage/kerjakuliah/' . $imgPreview) }}" class="img-fluid">
                            </div>
