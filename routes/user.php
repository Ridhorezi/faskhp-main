...
use App\Http\Livewire\User\LokerIndex;
use App\Http\Livewire\User\PaymentIndex;
...
Route::group(
    ['middleware' => ['role:user'], 'prefix' => 'user', 'as' => 'user.'],
    function () {
        Route::get('kerja', KerjaIndex::class)->name('kerja');
        Route::get('kuliah', KuliahIndex::class)->name('kuliah');
        Route::get('kerja_kuliah', KerjaKuliahIndex::class)->name(
            'kerjakuliah'
        );
        Route::get('mencari_kerja', MencariKerjaIndex::class)->name(
            'mencarikerja'
        );
        Route::get('usaha', UsahaIndex::class)->name('usaha');
        Route::get('loker', LokerIndex::class)->name('loker');
        ...
        Route::middleware('permission:participate')->group(function () {
            ...
            Route::get('add/kerjakuliah', AddKerjaKuliah::class)->name(
                'add.kerjakuliah'
            );
            Route::get('add/kuliah', AddKuliah::class)->name('add.kuliah');
            Route::get('add/mencari_kerja', AddMencariKerja::class)->name(
                'add.mencarikerja'
            );
            Route::get('add/usaha', AddUsaha::class)->name('add.usaha');
            Route::get('edit/kerja/{kerja}', EditKerjaUser::class)->name(
                'edit.kerja'
            );
            Route::get('edit/kuliah/{kuliah}', EditKuliahUser::class)->name(
                'edit.kuliah'
            );
            Route::get(
                'edit/kerja-kuliah/{kerjaKuliah}',
                EditKerjaKuliahUser::class
            )->name('edit.kerjakuliah');
            Route::get(
                'edit/mencari-kerja/{mencariKerja}',
                EditMencariKerjaUser::class
            )->name('edit.mencarikerja');
            Route::get('edit/usaha/{usaha}', EditUsahaUser::class)->name(
                'edit.usaha'
            );
        });
    }
