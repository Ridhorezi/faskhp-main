...
use App\Exports\KerjaKuliahExport;
...
class IndexKerjaKuliah extends Component
{
    ...
    {
        try {
            $find = KerjaKuliah::where('id', $this->selectedId)->firstOrFail();
            if ($find->gambar) {
    ...
    public function exportKerjaKuliah()
    {
        if ($this->items != null) {
            $this->emit('showAlert', 'success', 'Success export data');

            return (new KerjaKuliahExport($this->items))->download(
                'kerja-kuliah.xlsx'
            );
        } else {
            $this->emit(
    ...
    {
        $dataKerjaKuliah = $this->kerjaKuliahRender(KerjaKuliah::class)
            ->cleanRequest()
            ->paginate(10);

        return view(
            'livewire.tables.kerja-kuliah.index-kerja-kuliah',
            compact('dataKerjaKuliah')
