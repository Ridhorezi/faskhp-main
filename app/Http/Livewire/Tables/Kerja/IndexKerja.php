...
class IndexKerja extends Component
{
    ...
    {
        $this->setModel(Kerja::class);
        $this->hasPhoto(true, 'kerja');
    }
    ...
    public function exportKerja()
    {
        if ($this->items != null) {
            $this->emit('showAlert', 'success', 'Success export data');

            return (new KerjaExport($this->items))->download('kerja.xlsx');
        } else {
            $this->emit(
                'showAlert',
                'error',
    ...
    {
        $kerjas = $this->kerjaRender(Kerja::class)
            ->cleanRequest()
            ->paginate(10);

        return view('livewire.tables.kerja.index', compact('kerjas'))->layout(
