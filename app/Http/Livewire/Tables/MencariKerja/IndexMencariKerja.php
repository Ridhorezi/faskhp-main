...
use App\Exports\MencariKerjaExport;
...
class IndexMencariKerja extends Component
{
    ...
    public function exportMencariKerja()
    {
        if ($this->items != null) {
            $this->emit('showAlert', 'success', 'Success export data');

            return (new MencariKerjaExport($this->items))->download('mencari-kerja.xlsx');
        } else {
            $this->emit(
                'showAlert',
                'error',
    ...
    {
        $dataMencariKerja = $this->mencariKerjaRender(MencariKerja::class)->cleanRequest()->paginate(10);
