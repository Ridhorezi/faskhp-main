...
class EditMencariKerja extends Component
{
    ...
    protected $rules = [
        ...
        'kontak' => 'required|max:20|numeric',
        'gambar' =>
    ...
    {
        $currentData = MencariKerja::where(
            'id',
            $this->selectedId
        )->firstOrFail();
        $data = $this->validate();
        ...
        try {
            if (!$this->gambar) {
                ...
                if ($currentData->gambar) {
                    Storage::disk('public')->delete(
                        'mencarikerja/' . $currentData->gambar
                    );
                }
                $name =
                    time() .
            ...
            $this->emit(
                'showAlert',
                'error',
                "Data gagal di perbarui: {$e->getMessage()}"
            );
    ...
    {
        return view(
            'livewire.tables.mencari-kerja.edit-mencari-kerja'
