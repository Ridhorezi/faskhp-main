...
class RoleHelper
{
    ...
    public static function redirectByRoles(
        $userDestination,
        $superAdminDestination,
        $deactiveValiate = false
    ) {
        if (!$deactiveValiate) {
            self::validate();
        }
    ...
    public static function showViewByRoles(
        $userDestination,
        $superAdminDestination
    ) {
    ...
    public function render()
    {
        if (Auth::user()->hasRole('SuperAdmin')) {
            ...
            return view(self::$superAdminDestination, $this->viewData)->layout(
                $this->adminLayoutName,
                $this->adminLayoutData
            );
        }

        if ($this->userLayoutData) {
            return view(self::$userDestination, $this->viewData)->layout(
                $this->userLayoutName,
                $this->userLayoutData
            );
        } else {
