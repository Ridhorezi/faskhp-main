...
class IndexUsers extends Component
{
    ...
    public function upLevel($id)
    {
        try {
            ...
            $this->emit(
                'showAlert',
                'error',
                'Gagal menaikkan level. ' . $q->getMessage()
            );
            ...
            $this->emit(
                'showAlert',
                'error',
                'Gagal menaikkan level: ' . $e->getMessage()
            );
    ...
    public function exportUser()
    {
        if ($this->items != null) {
            ...
            return (new UserExport($this->items))->download('user.xlsx');
        } else {
            $this->emit(
                'showAlert',
                'error',
    ...
    {
        $users = $this->userRender(User::class)->paginate(10);

        return view('livewire.users.index-users', compact('users'))->layout(
